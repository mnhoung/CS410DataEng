...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "testing123"   # insert your postgres db password here
...
def addConstraintAndIndex(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Added index and constraint to {TableName}")
...
def load_copy(conn, filename, tablename):
	with conn.cursor() as cursor:
...
#	load(conn, cmdlist)
	load_copy(conn, Datafile, TableName)

	if CreateDB:
		addConstraintAndIndex(conn)
